...
COMMANDS = [
    {
        "name": "test_detector_canaries",
        "cmd": [sys.executable, "manage.py", "test", "tools.qa.test_detector_canaries", "--settings=config.settings.test", "--verbosity=2"]
    },
    {
        "name": "test_behavioral_discovery",
        "cmd": [sys.executable, "manage.py", "test", "tools.qa.test_behavioral_discovery", "--settings=config.settings.test", "--verbosity=2"]
    },
    {
        "name": "test_visual_matrix",
        "cmd": [sys.executable, "manage.py", "test", "tools.qa.test_visual_matrix", "--settings=config.settings.test", "--verbosity=2"]
    }
]

def parse_django_output(stdout, stderr, name):
    content = stdout + "\n" + stderr
...
def main():
    ...
    for item in COMMANDS:
        ...
        stdout_rel_path = f"tools/qa/reports/raw/{name}_stdout.log"
        stderr_rel_path = f"tools/qa/reports/raw/{name}_stderr.log"
        ...
        passed, failed, errors = parse_django_output(res.stdout, res.stderr, name)
        
        # Initialize default visual fields
        visual_findings_count = 0
        visual_expected_findings_count = 0
        visual_finding_ids = []
        visual_discovery_status = "N/A"
        
        # Determine status contribution and fill visual fields
        if name == "test_visual_matrix":
            # Parse visual findings from stdout
            ...
            # Check raw visual evidence validation
